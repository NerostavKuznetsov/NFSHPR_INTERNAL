...
struct tab_state
{
    ImVec4 background, icon, text;
};

bool ImGui::Tab(const char* label, const char* icon, bool selectable, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    it_anim->second.icon = ImLerp(it_anim->second.icon, selectable ? c::main_color : c::text_active, g.IO.DeltaTime * 14.f);

    it_anim->second.background = ImLerp(it_anim->second.background, selectable ? c::tab_active : c::tab_in_active, g.IO.DeltaTime * 14.f);

    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(it_anim->second.background), 8.f);

    PushFont(Inter_S_1);

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 50, bb.Min.y + 16), GetColorU32(c::text_active), label);

    PopFont();

	PushFont(Icon);

	GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 12, bb.Min.y + 12), GetColorU32(it_anim->second.icon), icon);
...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() || hovered ? ImColor(255, 246, 180) : ImColor(255, 246, 180), g.IO.DeltaTime * 8.f);

    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() || hovered ? ImColor(0,0,0,255) : ImColor(25, 25, 25, 255), g.IO.DeltaTime * 8.f);

    it_anim->second.size = ImLerp(it_anim->second.size, -30.f, g.IO.DeltaTime * 3.f);

    const ImU32 col = GetColorU32((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImGuiCol_Button);
    RenderNavHighlight(bb, id);
    //RenderFrame(bb.Min, bb.Max, GetColorU32(it_anim->second.background), true, 6);
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), 6.f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float square_sz = 24.f, w = (GetContentRegionMax().x - 16 - g.Style.WindowPadding.x) - square_sz;
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::main_color : c::checkbox_bg_in_active, g.IO.DeltaTime * 14.f);
    it_anim->second.circle = ImLerp(it_anim->second.circle, *v ? c::black : c::main_color, g.IO.DeltaTime * 14.f);

    it_anim->second.move = ImLerp(it_anim->second.move, *v ? 16.f : 0.f, g.IO.DeltaTime * 14.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    GetWindowDrawList()->AddRectFilled(check_bb.Min + ImVec2(w - 8, 2), check_bb.Max + ImVec2(w, -6), GetColorU32(it_anim->second.background), 100.f);

    GetWindowDrawList()->AddCircleFilled(ImVec2(check_bb.Min.x + (w + it_anim->second.move), check_bb.Max.y - 14), 5.f, GetColorU32(it_anim->second.circle), 100.f);

    GetWindowDrawList()->AddText(ImVec2(check_bb.Min.x + 16, check_bb.Min.y), GetColorU32(c::text_active), label);
...
struct Togle_state
{
    ImVec4 background, select;
};

bool ImGui::Togle_Button(const char* label, bool selectable, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    static std::map<ImGuiID, Togle_state> anim;
    auto it_anim = anim.find(id);

    if (it_anim == anim.end())
    {
        anim.insert({ id, Togle_state() });
        it_anim = anim.find(id);
    }

    it_anim->second.background = ImLerp(it_anim->second.background, selectable ? ImColor(101, 87, 255, 255) : ImColor(30, 31, 33, 89), g.IO.DeltaTime * 10.f);

    it_anim->second.select = ImLerp(it_anim->second.select, selectable ? c::select_tab_active : c::select_tab_inactive, g.IO.DeltaTime * 10.f);
    ...
    GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (size_arg.x - CalcTextSize(label).x) / 2, bb.Max.y - CalcTextSize(label).y - (size_arg.y - CalcTextSize(label).y) / 2), GetColorU32(c::text_active), label);

    return pressed;
}

bool ImGui::Checkbox_Togle(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    ...
    GetWindowDrawList()->AddText(ImVec2(check_bb.Min.x, check_bb.Min.y), GetColorU32(c::text_active), label);

    return pressed;
}
bool keybind_button(std::string_view label, bool* callback)
{
    struct keybind_button_state
    {
        float alpha = 0.f;
        ImVec4 label_clr = clr->key_button_label;
        ImVec4 text;
    };
    ...
    keybind_button_state* state = gui->anim_container(&state, id);

    state->text = ImLerp(state->text, *callback ? c::text_active : c::text_in_active, g.IO.DeltaTime * 14.f);

    state->alpha = ImClamp(state->alpha + (gui->fixed_speed(6.f) * (*callback ? 1.f : -1.f)), 0.f, 1.f);
    state->label_clr = ImLerp(state->label_clr, *callback ? clr->accent : clr->key_button_label, gui->fixed_speed(10.f));

    window->DrawList->AddRectFilled(rect.Min, rect.Max, gui->get_clr(clr->key_button_background, state->alpha),4.f);
    window->DrawList->AddText(rect.Min + ImVec2(6, 5), ImGui::GetColorU32(state->text), label.data());

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}

bool keybind_cbutton(std::string_view label, std::string_view icon, bool red)
{
    struct keybind_cbutton_state
    {
        float alpha = 0.f;
        bool active = false;
    };
    ...
    keybind_cbutton_state* state = gui->anim_container(&state, id);

    if (pressed)
        state->active = true;

    state->alpha = ImClamp(state->alpha + (gui->fixed_speed(6.f) * (state->active ? 1.f : -1.f)), 0.f, 1.f);
    ...
    //   window->DrawList->AddText(Inter_Bold_6, 16.f, rect.Min + ImVec2(12, 5), gui->get_clr(red ? clr->key_red_button_label : clr->key_button_label), label.data());
       //   window->DrawList->AddText(set->colorpicker_icon_font, set->colorpicker_icon_font->FontSize, rect.Min + ImVec2(8, icon == "D" ? 7 : 8), gui->get_clr(red ? clr->key_red_button_label : clr->key_button_label), icon.data());

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}
struct checkboxx_state
{
    bool clicked = false;
    float offset = 0.f;
    float alpha = 0.f;
    float alpha2 = 0.f;
    ImVec4 Text_color, background, rect, mark_color;
    bool active = false;
    bool active_1 = false;
    bool hovered = false;
    bool hovered_1 = false;
    float wnd_alpha = 0.f;
    float wnd_size = 0.f;
    float wnd_alpha_1 = 0.f;
    float wnd_size_1 = 0.f;
    float mark_size, mark_sizes, timer;
    ImVec2 stored_mouse_pos = ImVec2(0, 0);
    bool checks = false;
    float timer_value = 0.f;
    bool key_wnd_active = false;
    bool key_wnd_active_1 = false;
    float key_wnd_alpha = 0.f;
    float key_wnd_alpha_1 = 0.f;
    float key_wnd_size = 0.f;
    float key_wnd_size_1 = 0.f;
    float anim_alhpa, anim_alhpa_1 = 0.f;
    ImVec4 text, circle, circle_outline;
    float move;
};
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(11, 27), window->DC.CursorPos + ImVec2(w - 12, 67));
    const ImRect total_bb(bb.Min, bb.Max + ImVec2(label_size.x > 0.0f ? label_size.x : 0.0f, label_size.y + 10));
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 35) : 0.f, g.IO.DeltaTime * 14.f);
    it_anim->second.arrow_roll = ImLerp(it_anim->second.arrow_roll, it_anim->second.opened_combo ? -1.f : 1.f, g.IO.DeltaTime * 14.f);
    it_anim->second.text = ImLerp(it_anim->second.text, it_anim->second.opened_combo ? c::text_active : c::text_in_active, g.IO.DeltaTime * 14.f);
    it_anim->second.expand_rotation = ImLerp(it_anim->second.expand_rotation, it_anim->second.opened_combo ? -1.f : 1.f, g.IO.DeltaTime * 10.f);
    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);
  
    GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), GetColorU32(c::input), 4.f);

    ImRotateStart();
    //  GetWindowDrawList()->AddImage(image::arrows, bb.Min + ImVec2(w - 40, 10), bb.Min + ImVec2(w - 32, 16), ImVec2(0, 0), ImVec2(1, 1), GetColorU32(c::text_active));
    PushFont(Icon_Arrow);
    window->DrawList->AddText(ImVec2(bb.Max.x - 25, bb.Min.y + 17), GetColorU32(c::text_active), "W");
    PopFont();
    ImRotateEnd(1.57f * it_anim->second.expand_rotation);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(bb.Min + ImVec2(15, 12), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    PopStyleColor();

    window->DrawList->AddText(ImVec2(bb.Min.x + 5, bb.Min.y - 29), GetColorU32(c::text_active), label);
    ...
    it_anim->second.anim_alhpa = ImLerp(it_anim->second.anim_alhpa, hovered ? 1.f : 0.f, g.IO.DeltaTime * 10.f);
    it_anim->second.anim_alhpa_1 = ImLerp(it_anim->second.anim_alhpa_1, hovered ? 0.75f : 0.f, g.IO.DeltaTime * 10.f);
    //active_audio_notif = ImLerp(active_audio_notif, hovered ? true : false, g.IO.DeltaTime * 10.f);
    ...
    PushStyleColor(ImGuiCol_WindowBg, c::input);
...
bool ImGui::SliderScalar(const char* label,ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(16, label_size.y), window->DC.CursorPos + ImVec2(w - 10, label_size.y + 17.f + 3));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? label_size.x : 0.0f, 0.0f));

    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    ItemSize(ImRect(total_bb.Min, total_bb.Max + ImVec2(0, 20)));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? c::text_active : c::text_in_active, g.IO.DeltaTime * 14.f);
    ...
    GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 13), frame_bb.Max + ImVec2(-11, 2), GetColorU32(c::background_slider), 4.f);

    if (grab_bb.Max.x > grab_bb.Min.x) {

        it_anim->second.slow_anim = ImLerp(it_anim->second.slow_anim, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 25.f);

        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 13), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y + 4), GetColorU32(c::main_color), 4.f);

        GetWindowDrawList()->AddRectFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x - 3, grab_bb.Min.y + 6), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 5, grab_bb.Min.y + 24), GetColorU32(c::text_active), 4.f);
        GetWindowDrawList()->AddRectFilled(ImVec2(it_anim->second.slow_anim + frame_bb.Min.x - 1, grab_bb.Min.y + 8), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x + 3, grab_bb.Min.y + 22), GetColorU32(c::black), 4.f);
    }

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(frame_bb.Min - ImVec2(19, 24), frame_bb.Max - ImVec2(9, 22), value_buf, value_buf_end, NULL, ImVec2(1.0f, 1.0f));
    PopStyleColor();

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w + 26, frame_bb.Min.y - 19), GetColorU32(c::text_active), label);
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (InputText("", NULL, buf, IM_ARRAYSIZE(buf), flags)) // PushId(label) + "" gives us the expected ID from outside point of view
...
bool ImGui::InputTextEx(const char* label, ImTextureID image, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() - 156, (is_multiline ? g.FontSize * 8.0f : label_size.y + 5) + style.FramePadding.y * 4.0f); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), frame_size.y + 8);

    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, 6), window->DC.CursorPos + frame_size + ImVec2(0, 24));
    ...
    if (!is_multiline)
    {
        ...
        ImGui::GetBackgroundDrawList()->AddShadowRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), GetColorU32(it_anim->second.rect_multi_1),30.f,ImVec2(0,0), ImDrawFlags_ShadowCutOutShapeBackground,10.f);
        ImGui::GetForegroundDrawList()->AddRect(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x, frame_bb.Max.y), GetColorU32(it_anim->second.rect_multi_2), 10.f,0,2.f);

        //GetWindowDrawList()->AddRectFilledMultiColorRounded(ImVec2(frame_bb.Min.x - 1, frame_bb.Min.y - 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), GetColorU32(ImGuiCol_PopupBg), GetColorU32(it_anim->second.rect_multi_2), GetColorU32(it_anim->second.rect_multi_2), GetColorU32(it_anim->second.rect_multi_1), GetColorU32(it_anim->second.rect_multi_1), 6.f, ImDrawFlags_RoundCornersAll);
    }
    ...
    if (render_cursor || render_selection)
    {
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min + ImVec2(12, 10), rect.Max + ImVec2(15, 18), ImColor(30, 30, 30, 200));
                }
        ...
        {
            ImU32 col = GetColorU32(it_anim->second.text_color);

            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll + ImVec2(12, 14), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) {
                ...
                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_anim->second.value + 11, 18), frame_bb.Min + ImVec2(it_anim->second.value + 12, 39), ImColor(255, 255, 255));
            }
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ...
            ImU32 col = GetColorU32(it_anim->second.text_color);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos + ImVec2(12, 14), col, buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_PopupBg, GetColorU32(c::window));
        PushStyleColor(ImGuiCol_Border, GetColorU32(c::window));
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        ImGui::GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(16, 0), GetColorU32(c::text_active), label);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect bb(window->DC.CursorPos + ImVec2(w - 12, 2), window->DC.CursorPos + size + ImVec2(w  - 16, -3));
    ItemSize(ImRect(bb.Min, bb.Max + ImVec2(0, 4)));
    ...
    if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f)
    {
        ...
        window->DrawList->AddRectFilled(bb_inner.Min, ImVec2(mid_x, bb_inner.Max.y), GetColorU32(col_rgb_without_alpha), 4.f, ImDrawFlags_RoundCornersLeft);
    }
    else
    {
        ImVec4 col_source = (flags & ImGuiColorEditFlags_AlphaPreview) ? col_rgb : col_rgb_without_alpha;
        if (col_source.w < 1.0f)
            RenderColorRectWithAlphaCheckerboard(window->DrawList, bb_inner.Min, bb_inner.Max, GetColorU32(col_source), grid_step, ImVec2(off, off), 4);
        else
            window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max, GetColorU32(col_source), 4.f);
    }
...
struct selectable_state {
    ImVec4 text,circle,cirlce_active;
    float move;
};
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ItemSize(ImRect(bb.Min, bb.Max - ImVec2(0, 20)));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, selected ? c::text_active : c::text_in_active, g.IO.DeltaTime * 14.f);
    
    it_anim->second.move = ImLerp(it_anim->second.move, selected ? 8.f : 0.f, g.IO.DeltaTime * 14.f);

    it_anim->second.circle = ImLerp(it_anim->second.circle, selected ? c::circle_active : c::circle_inactive, g.IO.DeltaTime * 14.f);

    it_anim->second.cirlce_active = ImLerp(it_anim->second.cirlce_active, selected ? c::circle_selected : c::circle_in_selected, g.IO.DeltaTime * 14.f);
    ...
    GetWindowDrawList()->AddCircleFilled(bb.Max - ImVec2(23 + it_anim->second.move, 15),8.f, GetColorU32(it_anim->second.circle));

    GetWindowDrawList()->AddCircleFilled(bb.Max - ImVec2(23 + it_anim->second.move, 15), 4.f, GetColorU32(it_anim->second.cirlce_active));

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));

    RenderTextClipped(text_min + ImVec2(13 + it_anim->second.move, 10), text_max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo("##v", NULL, NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest);
    PopStyleColor(2);
